...
class TestSlimCommands < Minitest::Test
  ...
  def test_option_version
    ...
    assert err.empty?
    assert_match %r{\ASlim #{Regexp.escape Slim::VERSION}$}o, out
  end
  ...
  def test_rails
    prepare_common_test DYNAMIC_TEMPLATE, "--rails" do |out, err|
      ...
      if Gem::Version.new(Temple::VERSION) >= Gem::Version.new("0.9")
        assert out.include? %(@output_buffer = output_buffer || ActionView::OutputBuffer.new;)
      else
        assert out.include? %(@output_buffer = ActiveSupport::SafeBuffer.new;)
      end
      assert out.include? %{@output_buffer.safe_concat(("<p>Hello ".freeze));}
      assert out.include? %{@output_buffer.safe_concat(((::Temple::Utils.escape_html((name))).to_s));}
      assert out.include? %{@output_buffer.safe_concat(("!</p>".freeze));}
    end
  end
