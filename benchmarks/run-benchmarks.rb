...
class RailsController < AbstractController::Base
...
class SlimBenchmarks
  def initialize(slow, iterations)
    ...
    @instances_erb_filename = File.dirname(__FILE__) + '/instances_view'
    @erb_filename = File.dirname(__FILE__) + '/view'
    @erb_code  = File.read("#{@erb_filename}.erb")
  ...
  def init_compiled_benches
    ...
    bench('(1) actionview locals erubis')  { controller.show_with_locals @erb_filename, context }
    bench('(1) actionview vars erubis')    { controller.show_with_vars @instances_erb_filename, context }
